"""VulnHawk rpc_client for static windows app analysis."""
import base64
...
def _init_key():
    ...
    pub_key = rsa.PublicKey.load_pkcs1(
        open(config['VulnHawk']['pub_key']).read())
...
def binskim(sample, signature):
    ...
    command = 'analyze'
    path = config['VulnHawk']['samples'] + sample
    output_p = '-o'
    output_d = config['VulnHawk']['samples'] + sample + '_binskim'
    verbose = '--verbose'
    policy_p = '--config'
...
def binscope(sample, signature):
    ...
    target = [config['VulnHawk']['samples'] + sample]
    out_type = ['/Red', '/v']
