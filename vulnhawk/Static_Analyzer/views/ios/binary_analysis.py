...
from vulnhawk.Static_Analyzer.views.ios.classdump import (
    get_class_dump,
)
from vulnhawk.Static_Analyzer.views.common.binary.lib_analysis import (
    MachOChecksec,
)
from vulnhawk.Static_Analyzer.views.common.binary.strings import (
    strings_on_binary,
)
...
def binary_analysis(checksum, src, tools_dir, app_dir, executable_name):
    ...
    try:
        ...
        if not (bin_path.exists() and bin_path.is_file()):
            msg = (
                f'VulnHawk Cannot find binary in {bin_path.as_posix()}. '
