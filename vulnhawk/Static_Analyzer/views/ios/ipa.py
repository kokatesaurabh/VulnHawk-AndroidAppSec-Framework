...
from vulnhawk.Static_Analyzer.views.ios.appstore import app_search
from vulnhawk.Static_Analyzer.views.ios.binary_analysis import (
    binary_analysis,
)
from vulnhawk.Static_Analyzer.views.common.binary.lib_analysis import (
    library_analysis,
)
from vulnhawk.Static_Analyzer.views.ios.code_analysis import ios_source_analysis
from vulnhawk.Static_Analyzer.views.ios.db_interaction import (
    get_context_from_db_entry,
    save_get_ctx,
)
from vulnhawk.Static_Analyzer.views.ios.file_analysis import ios_list_files
from vulnhawk.Static_Analyzer.views.ios.icon_analysis import (
    get_icon_from_ipa,
    get_icon_source,
)
from vulnhawk.Static_Analyzer.views.ios.plist_analysis import (
    get_plist_secrets,
    plist_analysis,
)
from vulnhawk.Static_Analyzer.views.ios.strings import (
...
from vulnhawk.Malware_Analyzer.views.MalwareDomainCheck import (
    MalwareDomainCheck,
)
...
def ipa_analysis_task(checksum, app_dic, rescan, queue=False):
    ...
    try:
        ...
        if not extract_and_check_ipa(checksum, app_dic):
            msg = ('IPA is malformed! VulnHawk cannot find Payload directory')
...
def ipa_analysis(request, app_dic, rescan, api):
    ...
    ipa_db = Static_AnalyzerIOS.objects.filter(MD5=checksum)
...
def ios_analysis(request, app_dic, rescan, api):
    ...
    ios_zip_db = Static_AnalyzerIOS.objects.filter(MD5=checksum)
