...
from vulnhawk.VulnHawk import settings
...
from vulnhawk.Static_Analyzer.views.windows.db_interaction import (
...
def pdf(request, checksum, api=False, jsonres=False):
    try:
        ...
        # Do Lookups
        android_static_db = Static_AnalyzerAndroid.objects.filter(
            MD5=checksum)
        ios_static_db = Static_AnalyzerIOS.objects.filter(
            MD5=checksum)
        win_static_db = Static_AnalyzerWindows.objects.filter(
            MD5=checksum)
