...
from vulnhawk.VulnHawk.views.home import update_scan_timestamp
...
from vulnhawk.Static_Analyzer.tools.strings import strings_util
...
def staticanalyzer_windows(request, checksum, api=False):
    ...
    try:
        ...
        app_dic['tools_dir'] = os.path.join(
            settings.BASE_DIR, 'Static_Analyzer/tools/windows/')
        # DB
        db_entry = Static_AnalyzerWindows.objects.filter(
            MD5=checksum,
        )
...
def _binary_analysis(app_dic):
    ...
    if platform.system() != 'Windows' or 'CI' in os.environ:
        ...
        config.read(expanduser('~') + '\\VulnHawk\\Config\\config.txt')
