...
import vulnhawk.Malware_Analyzer.views.Trackers as Trackers
import vulnhawk.Malware_Analyzer.views.VirusTotal as VirusTotal
from vulnhawk.Malware_Analyzer.views.android import (
    apkid,
    permissions,
)
from vulnhawk.Malware_Analyzer.views.MalwareDomainCheck import MalwareDomainCheck

from django.conf import settings
from django.http import HttpResponseRedirect
from django.shortcuts import render

from vulnhawk.VulnHawk.utils import (
    append_scan_status,
    file_size,
    print_n_send_error_response,
)
from vulnhawk.Static_Analyzer.models import (
    Static_AnalyzerAndroid,
    Static_AnalyzerIOS,
)
from vulnhawk.Static_Analyzer.views.common.binary.lib_analysis import (
    library_analysis,
)
from vulnhawk.Static_Analyzer.views.android.app import (
    aapt_parse,
    androguard_parse,
    get_apk_name,
)
from vulnhawk.Static_Analyzer.views.android.cert_analysis import (
    cert_info,
    get_hardcoded_cert_keystore,
)
from vulnhawk.Static_Analyzer.views.android.code_analysis import code_analysis
from vulnhawk.Static_Analyzer.views.android.converter import (
    apk_2_java,
    dex_2_smali,
)
from vulnhawk.Static_Analyzer.views.android.db_interaction import (
    get_context_from_db_entry,
    save_get_ctx,
)
from vulnhawk.Static_Analyzer.views.android.icon_analysis import (
    get_icon_apk,
    get_icon_from_src,
)
from vulnhawk.Static_Analyzer.views.android.manifest_analysis import (
    manifest_analysis,
)
from vulnhawk.Static_Analyzer.views.android.manifest_utils import (
    extract_manifest_data,
    get_parsed_manifest,
)
from vulnhawk.Static_Analyzer.views.android.playstore import (
    get_app_details,
)
from vulnhawk.Static_Analyzer.views.android.strings import (
    get_strings_metadata,
)
from vulnhawk.Static_Analyzer.views.common.shared_func import (
    get_avg_cvss,
    hash_gen,
    unzip,
)
from vulnhawk.Static_Analyzer.views.common.firebase import (
    firebase_analysis,
)
from vulnhawk.Static_Analyzer.views.common.appsec import (
    get_android_dashboard,
)
from vulnhawk.Static_Analyzer.views.common.async_task import (
    async_analysis,
    enqueued_task_init,
    update_enqueued_task,
)
from vulnhawk.VulnHawk.views.authorization import (
    Permissions,
    has_permission,
)
...
def apk_analysis(request, app_dic, rescan, api):
    ...
    db_entry = Static_AnalyzerAndroid.objects.filter(MD5=checksum)
...
def src_analysis(request, app_dic, rescan, api):
    ...
    db_entry = Static_AnalyzerAndroid.objects.filter(
        MD5=checksum)
    ios_db_entry = Static_AnalyzerIOS.objects.filter(
        MD5=checksum)
    if db_entry.exists() and not rescan:
        context = get_context_from_db_entry(db_entry)
