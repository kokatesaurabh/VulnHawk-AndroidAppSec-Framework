...
from vulnhawk.Static_Analyzer.models import RecentScansDB
...
# VulnHawk Custom regex to catch maximum URI like strings
...
def print_version():
    """Print VulnHawk Version."""
    logger.info(settings.BANNER)
    ver = settings.MOBSF_VER
    logger.info('Author: Saurabh Kokate | opensecurity.in')
    vulnhawk_api_key = api_key(settings.MOBSF_HOME)
    if platform.system() == 'Windows':
        logger.info('VulnHawk-AndroidAppSec-Frameworkk %s', ver)
        print(f'REST API Key: {vulnhawk_api_key}')
        print('Default Credentials: vulnhawk/vulnhawk')
    else:
        logger.info(
            '%sVulnHawk-AndroidAppSec-Frameworkk %s%s', Color.GREY, ver, Color.END)
        print(f'REST API Key: {Color.BOLD}{vulnhawk_api_key}{Color.END}')
        print(f'Default Credentials: {Color.BOLD}vulnhawk/vulnhawk{Color.END}')
    os = platform.system()
...
def check_update():
    try:
        ...
        if remote_version:
            ...
            if sem_loc < sem_rem:
                logger.warning('A new version of VulnHawk is available, '
...
def check_basic_env():
    """Check if we have basic env for VulnHawk to run."""
    logger.info('VulnHawk Basic Environment Check')
    try:
...
def get_android_dm_exception_msg():
    return (
        'Is your Android VM/emulator running? VulnHawk cannot'
