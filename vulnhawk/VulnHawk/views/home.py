# -*- coding: utf_8 -*-
"""VulnHawk File Upload and Home Routes."""
import json
import logging
import os
import platform
...
from vulnhawk.VulnHawk.forms import FormUtil, UploadFileForm
...
from vulnhawk.VulnHawk.init import api_key
from vulnhawk.VulnHawk.security import sanitize_filename
from vulnhawk.VulnHawk.views.helpers import FileType
from vulnhawk.VulnHawk.views.scanning import Scanning
from vulnhawk.VulnHawk.views.apk_downloader import apk_download
from vulnhawk.Static_Analyzer.models import (
    EnqueuedTask,
    RecentScansDB,
    Static_AnalyzerAndroid,
    Static_AnalyzerIOS,
    Static_AnalyzerWindows,
)
from vulnhawk.Dynamic_Analyzer.views.common.shared import (
    invalid_params,
    send_response,
)
from vulnhawk.VulnHawk.views.authentication import (
    login_required,
)
...
def recent_scans(request, page_size=10, page_number=1):
    ...
    android = Static_AnalyzerAndroid.objects.filter(
        MD5__in=md5_list).only(
            'PACKAGE_NAME', 'VERSION_NAME', 'FILE_NAME', 'MD5')
    ios = Static_AnalyzerIOS.objects.filter(
    ...
    for entry in page_obj:
        ...
        if entry['FILE_NAME'].endswith('.ipa'):
            ...
            report_file = updir / entry['BUNDLE_HASH'] / 'vulnhawk_dump_file.txt'
        else:
...
def download(request):
    """Download from vulnhawk downloads directory."""
...
def delete_scan(request, api=False):
    ...
    try:
        ...
        Static_AnalyzerAndroid.objects.filter(MD5=md5_hash).delete()
        Static_AnalyzerIOS.objects.filter(MD5=md5_hash).delete()
        Static_AnalyzerWindows.objects.filter(MD5=md5_hash).delete()
