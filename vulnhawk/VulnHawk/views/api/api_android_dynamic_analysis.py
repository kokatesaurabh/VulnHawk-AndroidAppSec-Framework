...
from vulnhawk.Dynamic_Analyzer.views.common import (
    device,
    frida,
)
...
@request_method(['POST'])
@csrf_exempt
def api_vulnhawky(request):
    """POST - VulnHawky API."""
    if 'identifier' not in request.POST:
        return make_api_response(
            {'error': 'Missing Parameters'}, 422)
    resp = operations.vulnhawky(request, True)
    if resp['status'] == 'ok':
        return make_api_response(resp, 200)
    return make_api_response(resp, 500)
...
def api_root_ca(request):
    """POST - VulnHawk CA actions API."""
    if 'action' not in request.POST:
        return make_api_response(
            {'error': 'Missing Parameters'}, 422)
    resp = operations.vulnhawk_ca(request, True)
    if resp['status'] == 'ok':
        return make_api_response(resp, 200)
    return make_api_response(resp, 500)
...
def api_global_proxy(request):
    """POST - VulnHawk Global Proxy API."""
    if 'action' not in request.POST:
        return make_api_response(
            {'error': 'Missing Parameters'}, 422)
