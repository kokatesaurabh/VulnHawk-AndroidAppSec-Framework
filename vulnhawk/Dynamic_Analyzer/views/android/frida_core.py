...
from vulnhawk.Dynamic_Analyzer.views.android.environment import Environment
from vulnhawk.Dynamic_Analyzer.views.android.frida_scripts import (
    class_pattern,
    class_trace,
    get_loaded_classes,
    get_methods,
...
class Frida:
    ...
    def __init__(self, app_hash, package, defaults, auxiliary, extras, code):
        ...
        self.api_mon = self.apk_dir / 'vulnhawk_api_monitor.txt'
        self.frida_log = self.apk_dir / 'vulnhawk_frida_out.txt'
        self.deps = self.apk_dir / 'vulnhawk_app_deps.txt'
        self.clipboard = self.apk_dir / 'vulnhawk_app_clipboard.txt'
    ...
    def frida_response(self, message, data):
        ...
        if 'payload' in message:
            msg = message['payload']
            api_mon = 'VulnHawk-API-Monitor: '
            aux = '[AUXILIARY] '
            deps = '[RUNTIME-DEPS] '
            clip = 'vulnhawk-android-clipboard:'
