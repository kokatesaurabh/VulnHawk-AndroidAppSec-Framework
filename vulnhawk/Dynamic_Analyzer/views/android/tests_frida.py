...
from vulnhawk.Dynamic_Analyzer.views.android.frida_core import Frida
...
def live_api(request, api=False):
    try:
        ...
        if stream:
            ...
            apimon_file = os.path.join(apk_dir, 'vulnhawk_api_monitor.txt')
            data = {}
...
def apimon_analysis(app_dir):
    ...
    try:
        strings = []
        location = os.path.join(app_dir, 'vulnhawk_api_monitor.txt')
...
def dependency_analysis(package, app_dir):
    ...
    try:
        ...
        location = Path(app_dir) / 'vulnhawk_app_deps.txt'
