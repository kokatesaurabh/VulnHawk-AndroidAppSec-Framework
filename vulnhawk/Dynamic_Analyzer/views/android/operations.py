...
def vulnhawky(request, api=False):
    """Configure Instance for Dynamic Analysis."""
    logger.info('VulnHawkying Android instance')
    data = {}
    ...
    try:
        ...
        if not create_env.connect_n_mount():
            ...
            return send_response(data, api)
        version = create_env.vulnhawky_init()
        if not version:
            data = {'status': 'failed', 'message': msg}
            return send_response(data, api)
        else:
        ...
        logger.exception('VulnHawkying Android instance failed')
...
@permission_required(Permissions.SCAN)
@require_http_methods(['POST'])
def vulnhawk_ca(request, api=False):
    """Install and Remove VulnHawk Proxy RootCA."""
    data = {}
    try:
        env = Environment()
        action = request.POST['action']
        if action == 'install':
            env.install_vulnhawk_ca(action)
        ...
        logger.exception('VulnHawk RootCA Handler')
        data = {'status': 'failed', 'message': str(exp)}
    return send_response(data, api)
# AJAX
...
def global_proxy(request, api=False):
    ...
    try:
        ...
        logger.exception('VulnHawk Global Proxy')
        data = {'status': 'failed', 'message': str(exp)}
    return send_response(data, api)
