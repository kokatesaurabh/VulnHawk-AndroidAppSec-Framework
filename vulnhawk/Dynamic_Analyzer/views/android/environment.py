...
from vulnhawk.Dynamic_Analyzer.tools.webproxy import (
    create_ca,
    get_ca_file,
    get_http_tools_url,
    start_proxy,
...
from vulnhawk.Dynamic_Analyzer.views.android import (
...
from vulnhawk.Static_Analyzer.models import Static_AnalyzerAndroid
...
class Environment:
    ...
    def __init__(self, identifier=None):
        ...
        self.frida_str = f'VulnHawk-Frida-{frida_version}'.encode('utf-8')
        self.xposed_str = b'VulnHawk-Xposed'
    ...
    def configure_proxy(self, project, request):
        """HTTPS Proxy."""
        self.install_vulnhawk_ca('install')
    ...
    def install_vulnhawk_ca(self, action):
        """Install or Remove VulnHawk Root CA."""
        vulnhawk_ca = get_ca_file()
        ca_file = None
        if is_file_exists(vulnhawk_ca):
            ca_construct = '{}.0'
            pem = open(vulnhawk_ca, 'rb')
            ...
            return
        if action == 'install':
            logger.info('Installing VulnHawk RootCA')
            ...
            logger.info('Removing VulnHawk RootCA')
            self.adb_command(['rm',
                              ca_file], True)
    ...
    def android_component(self, bin_hash, comp):
        """Get APK Components."""
        try:
            anddb = Static_AnalyzerAndroid.objects.get(MD5=bin_hash)
            resp = []
            if comp == 'activities':
    ...
    def is_vulnhawkyied(self, android_version):
        """Check is Device is VulnHawkyed."""
        logger.info('Environment VulnHawkyed Check')
        if android_version < 5:
            agent_file = '.vulnhawk-x'
            agent_str = self.xposed_str
        else:
            agent_file = '.vulnhawk-f'
    ...
    def vulnhawky_init(self):
        ...
        try:
            if version < 5:
                ...
                self.vulnhawk_agents_setup('xposed')
            else:
                self.frida_setup()
                self.vulnhawk_agents_setup('frida')
            logger.info('VulnHawkying Completed!')
            return version
        except Exception:
            logger.exception('Failed to VulnHawky Android Instance')
            return False

    def vulnhawk_agents_setup(self, agent):
        """Setup VulnHawk agents."""
        create_ca()
        # Install MITM RootCA
        self.install_vulnhawk_ca('install')
        if agent == 'frida':
    ...
    def xposed_setup(self, android_version):
        ...
        # Install VulnHawk Agents for Xposed
        clip_dump_apk = os.path.join(self.tools_dir,
                                     xposed_dir,
                                     'ClipDump.apk')
        logger.info('Installing VulnHawk Clipboard Dumper')
