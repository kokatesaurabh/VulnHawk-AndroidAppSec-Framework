...
from vulnhawk.Dynamic_Analyzer.views.android.frida_core import (
    Frida,
)
...
def run_tls_tests(request, md5_hash, env, package, test_pkg, duration):
    ...
    env.install_vulnhawk_ca('remove')
    env.run_app(package)
    env.wait(duration)
    ...
    # Test 3: VulnHawk TLS Pinning Bypass Check
    env.adb_command(['am', 'force-stop', package], True)
    ...
    env.adb_command(['am', 'force-stop', package], True)
    out = Path(settings.UPLD_DIR) / md5_hash / 'vulnhawk_tls_tests.json'
