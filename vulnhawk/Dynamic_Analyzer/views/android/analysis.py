...
def run_analysis(apk_dir, md5_hash, package):
    ...
    clip = Path(apk_dir) / 'vulnhawk_app_clipboard.txt'
    if clip.exists():
            ...
            for img in os.listdir(screen_dir):
                if img.endswith('.png'):
                    if img.startswith('act'):
                        ...
                        imgs.append(img)
            try:
                sadb = Static_AnalyzerAndroid.objects.get(MD5=md5_hash)
...
def get_tls_logs(apk_dir, md5_hash):
    ...
    out = Path(apk_dir) / 'vulnhawk_tls_tests.json'
    if not out.exists():
        return None
...
def get_log_data(apk_dir, package):
    ...
    apimon = os.path.join(apk_dir, 'vulnhawk_api_monitor.txt')
    fd_logs = os.path.join(apk_dir, 'vulnhawk_frida_out.txt')
