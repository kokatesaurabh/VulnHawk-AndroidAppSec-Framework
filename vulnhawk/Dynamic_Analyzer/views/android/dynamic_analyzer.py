...
from vulnhawk.Dynamic_Analyzer.views.android.environment import (
    ANDROID_API_SUPPORTED,
    Environment,
)
from vulnhawk.Dynamic_Analyzer.views.android.operations import (
    get_package_name,
)
...
from vulnhawk.VulnHawk.views.scanning import add_to_recent_scan
...
def android_dynamic_analysis(request, api=False):
    ...
    try:
        ...
        and_ver = None
        and_sdk = None
        apks = Static_AnalyzerAndroid.objects.filter(
            APP_TYPE='apk')
        ...
        context = {'apps': scan_apps,
                   ...
                   'title': 'VulnHawk Dynamic Analysis',
...
def dynamic_analyzer(request, checksum, api=False):
    ...
    try:
        ...
        try:
            static_android_db = Static_AnalyzerAndroid.objects.get(
                MD5=checksum)
        ...
        if not env.is_vulnhawkyied(version):
            msg = ('This Android instance is not VulnHawkyed/Outdated.\n'
                   'VulnHawkying the android runtime environment')
            logger.warning(msg)
            if not env.vulnhawky_init():
                return print_n_send_error_response(
                    request,
                    'Failed to VulnHawky the instance',
                    api)
            if version < 5:
        ...
        if xposed_first_run:
            msg = ('Have you VulnHawkyed the instance before'
